...
(async () => {
  ...
  if (basename === 'visual_map.csv') {
    const required = ['06_retention_map.md', 'production_notes.md'];
    const missing = missingFiles(episodeDir, required);
    if (missing.length) {
      emitBlock('Cannot build visual_map.csv before brief and retention are complete.', missing);
      return;
    }
  }

  if (basename === 'production_notes.md' && !exists(path.join(episodeDir, '08_chapter_briefs.md'))) {
    emitBlock('Cannot update production_notes.md for handoff before 08_chapter_briefs.md exists.');
  }
})().catch(err => {
  process.stdout.write(JSON.stringify({
    continue: false,
