...
async function run() {
    ...
    try {
        ...
        const filesToDownload = ['types.ts', 'App.tsx', 'components/Sidebar.tsx', 'components/Monitor.tsx', 'components/PromptTable.tsx'];
        ...
        for (const filename of filesToDownload) {
            ...
            // Tìm file ở cột trái (so sánh chính xác tên đường dẫn đầy đủ)
            const fileCoords = await flowPage.evaluate((fname) => {
                const elements = Array.from(document.querySelectorAll('span, div, button'));
                
                // So sánh bằng chính xác textContent
                const fileEl = elements.find(el => {
                    const rect = el.getBoundingClientRect();
                    const text = el.textContent?.trim() || '';
                    // Có thể text chứa icon hoặc chữ description, ta dùng includes hoặc trim
                    return rect.left < window.innerWidth * 0.4 && 
                           (text === fname || text.endsWith(fname) || text.includes(fname));
                });

                if (fileEl) {
                    const rect = fileEl.getBoundingClientRect();
                    return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2, foundText: fileEl.textContent?.trim() };
                }
                return null;
            }, filename);

            if (fileCoords) {
                console.log(`Click file "${filename}" (Text tìm thấy: "${fileCoords.foundText}") tại tọa độ: (${fileCoords.x}, ${fileCoords.y})`);
                await flowPage.mouse.click(fileCoords.x, fileCoords.y);
                await delay(3000); // Chờ code load

                // Lấy code từ thẻ PRE hiển thị
