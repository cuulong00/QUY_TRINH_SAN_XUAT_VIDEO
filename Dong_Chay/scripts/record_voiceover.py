#!/usr/bin/env python3
"""
Bridge Caller for Production Voiceover (TTS) in Dong_Chay.
Calls the centralized engine at /Users/pro16/Documents/Code/TTS/run_production.py
without duplicating TTS dependencies or phonetic libraries in Dong_Chay.
"""
...
DONGCHAY_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

def resolve_episode_dir(episode_arg):
    """Resolve episode input to absolute path within Dong_Chay."""
    ...
    # Check episodes/<slug>
    ep_path = os.path.join(DONGCHAY_ROOT, "episodes", episode_arg)
    if os.path.isdir(ep_path):
        return ep_path
        
    # Check if slug given as 'episodes/slug'
    ep_path2 = os.path.join(DONGCHAY_ROOT, episode_arg)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Dong_Chay Production TTS Caller (delegates to Code/TTS)",
        formatter_class=argparse.RawTextHelpFormatter
    )
    parser.add_argument("episode", type=str,
                        help="Episode slug or directory (e.g. 'tap-doan-dai-dung' or 'episodes/tap-doan-dai-dung')")
    parser.add_argument("--voice", type=str, default="voice_mau_dong_chay_dai",
                        help="Voice preset ID (default: voice_mau_dong_chay_dai)")
    parser.add_argument("--speed", type=float, default=0.95,
                        help="Speaking speed tempo (default: 0.95)")
    ...
    if not episode_dir:
        ...
        print(f"Searched under: {os.path.join(DONGCHAY_ROOT, 'episodes')}")
        sys.exit(1)
    ...
    print(f"🎙️ [Dong_Chay Bridge] Calling Code/TTS Engine...")
