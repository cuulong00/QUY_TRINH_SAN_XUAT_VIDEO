...
Trong hệ thống Góc Nhìn Podcast, cấu hình mặc định là:
...
2. **Nhập tài nguyên (Import Assets)**:
   - Import toàn bộ file audio voiceover (`chapter_01.wav`, `chapter_02.wav`...) từ thư mục `audio/` đã được thu âm từ OmniVoice TTS.
...
*(Lưu ý kỹ thuật: Script cũ vẫn được giữ lại trong repo như một công cụ legacy dự phòng, nhưng quy trình chính thức từ nay là manual CapCut).*
