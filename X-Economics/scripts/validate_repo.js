...
const SHORT_DISCLAIMER = 'Nội dung chia sẻ góc nhìn khách quan, mang tính thảo luận và xây dựng';
const LONG_DISCLAIMER_SNIPPET = 'chia sẻ góc nhìn khách quan';
...
function validateCommands(reporter) {
  ...
  const checks = [
    ...
    ['editorial_qa.md', ['compliance', 'moralizing', 'fabricated']],
...
function validateEpisodes(reporter, specificSlug) {
  ...
  for (const slug of episodeNames) {
    ...
    const requirements = [
      ...
      { name: 'Editorial & Legal QA', phase: 'editorial_qa', files: ['editorial_qa.md'] },
    ...
    const isEarlyPhase = !['final_merge', 'editorial_qa', 'oral_qa', 'visual_map', 'slideshow_render', 'production_handoff', 'postmortem'].includes(currentPhase);
    ...
    validateEditorialQa(reporter, slug, episodeDir);
  }
}
...
function validateClaimLedger(reporter, slug, episodeDir, currentPhase, isLegacy, isPreExisting) {
  ...
  if (!claimsTable) {
    if (['research_map', 'hook_lab', 'thesis_map', 'retention_map', 'outline', 'chapter_briefs', 'chapter_writing', 'final_merge', 'editorial_qa', 'oral_qa', 'visual_map', 'slideshow_render', 'production_handoff', 'postmortem'].includes(currentPhase)) {
...
function validateEditorialQa(reporter, slug, episodeDir) {
  const qaPath = path.join(episodeDir, 'editorial_qa.md');
  const finalPath = path.join(episodeDir, 'final_voiceover.md');
  ...
  const qaSaysPresent = /^compliance_note_present:\s*yes\s*$/m.test(qa);
  const qaSaysAbsent = /^compliance_note_present:\s*no\s*$/m.test(qa);

  if (hasDisclaimer && qaSaysAbsent) {
    reporter.warn(`episode:${slug}`, 'editorial_qa.md says compliance_note_present: no but final_voiceover.md contains a compliance note.');
  }
  if (!hasDisclaimer && qaSaysPresent) {
    reporter.warn(`episode:${slug}`, 'editorial_qa.md says compliance_note_present: yes but final_voiceover.md does not contain a compliance note.');
  }
}
