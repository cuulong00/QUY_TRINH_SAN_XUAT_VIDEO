...
export class SharedContextManager {
  ...
  private async recreateContext(overrideHeadless?: boolean): Promise<void> {
    ...
    if (statePath) {
      try {
        // Manually inject cookies because launchPersistentContext ignores storageState option
        if (fs.existsSync(statePath)) {
          try {
            const state = JSON.parse(fs.readFileSync(statePath, 'utf-8'));
            if (state && Array.isArray(state.cookies) && state.cookies.length > 0) {
              await this.globalContext.addCookies(state.cookies);
              log.success(`  ✅ Manually injected ${state.cookies.length} cookies into persistent context`);
            }
          } catch (err) {
            log.warning(`  ⚠️ Failed to manually inject cookies: ${err}`);
          }
        }
  ...
  private async prepareIsolatedProfileDir(baseProfile: string): Promise<string> {
    ...
    const destProfileDir = path.join(dir, 'Default');
    try {
      ...
      if (CONFIG.cloneProfileOnIsolated && fs.existsSync(baseProfile)) {
        ...
        await (fs.promises as any).cp(baseProfile, destProfileDir, {
          recursive: true,
          errorOnExist: false,
          force: true,
          filter: (src: string) => {
