...
function writeStatusMarkdown(filePath, result) {
  ...
  for (const attempt of result.attempts || []) {
    lines.push(`- backend: ${attempt.backend || 'unknown'}`);
    lines.push(`  variant: ${attempt.variant || 'unknown'}`);
  ...
  lines.push(
    ...
    '- Hãy dùng metadata/status artifact này để quyết định: thử lại sau, đổi video, hoặc yêu cầu transcript thủ công từ user.'
  );
