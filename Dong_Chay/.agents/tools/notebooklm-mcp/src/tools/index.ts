...
User: "Yes" → call remove_notebook`,
    ...
    },
    {
      name: 'deep_research',
      description:
        'Execute Deep Research on a NotebookLM notebook.\n\n' +
        'This tool uses NotebookLM\'s built-in "Deep Research" feature to automatically search the web, ' +
        'gather comprehensive information about a topic, and add the findings as source documents to ' +
        'the notebook. After completion, you can use ask_question to query the research results.\n\n' +
        '## ⛔ CRITICAL: notebook_url is REQUIRED\n' +
        'You MUST provide notebook_url to target an existing notebook. If omitted, the tool falls back ' +
        'to the active notebook. If no active notebook is set, the tool will REFUSE to run.\n' +
        'DO NOT call deep_research without specifying which notebook to use.\n' +
        'Rule: 1 VIDEO = 1 MASTER NOTEBOOK. All research queries go into the SAME notebook.\n\n' +
        '## When To Use\n' +
        '- When a notebook needs additional web research on a specific topic\n' +
        '- When you need comprehensive, multi-source research on a topic\n' +
        '- Before asking questions, to populate the notebook with relevant data\n\n' +
        '## Workflow\n' +
        '1) Call deep_research({ query: "your research topic", notebook_url: "https://notebooklm.google.com/notebook/..." })\n' +
        '2) Wait for research to complete (1-5 minutes)\n' +
        '3) Call ask_question to analyze the gathered sources\n\n' +
        '## Example\n' +
        'deep_research({ query: "VinFast financial reports 2024-2026", notebook_url: "https://notebooklm.google.com/notebook/abc123" })\n' +
        '→ Returns: { success: true, sourcesAdded: 5, message: "..." }\n' +
        'Then: ask_question({ question: "Summarize VinFast funding structure", notebook_url: "https://notebooklm.google.com/notebook/abc123" })\n\n' +
        'Note: Deep Research can take 1-5 minutes. Timeout defaults to 10 minutes.',
      inputSchema: {
        type: 'object',
        properties: {
          query: {
            type: 'string',
            description: 'The research topic or question to investigate via Deep Research',
          },
          notebook_url: {
            type: 'string',
            description:
              '⛔ EFFECTIVELY REQUIRED. The target notebook URL. All research goes into this ONE notebook. ' +
              'If omitted, falls back to active notebook. If no active notebook → tool REFUSES to run. ' +
              'DO NOT omit this unless you have already set an active notebook via select_notebook.',
          },
          session_id: {
            type: 'string',
            description:
              'Optional session ID to reuse an existing browser session.',
          },
          timeout_ms: {
            type: 'number',
            description:
              'Timeout in milliseconds for research completion (default: 600000 = 10 minutes)',
          },
          show_browser: {
            type: 'boolean',
            description: 'Show browser window during research. Default: false (headless).',
          },
        },
        required: ['query'],
      },
...
export class ToolHandlers {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          try {
            const screenshotPath = path.join(CONFIG.dataDir, 'debug-homepage.png');
            await page.screenshot({ path: screenshotPath, fullPage: true });
            log.warning(`  📸 Screenshot saved to ${screenshotPath}`);
          } catch (screenshotError) {
            log.warning(`  ⚠️ Could not save screenshot: ${screenshotError}`);
          }
        }
  ...
  > {
    ...
    try {
      ...
      try {
        ...
        // Look for "Create", "Créer" or "Tạo" button
        const createButtonSelectors = [
          'button:has-text("Create")',
          'button:has-text("Créer")',
          'button:has-text("Tạo")',
          'button:has-text("Mới")',
          'button:has-text("Tạo sổ ghi chú")',
        ...
        if (!clicked) {
          ...
          for (const btn of allButtons) {
            ...
            if (text && (text.includes('Create') || text.includes('Créer') || text.includes('Tạo') || text.includes('Mới') || text.includes('+'))) {
              await btn.click();
              clicked = true;
  ...
  async handleDeepResearch(args: {
    ...
    timeout_ms?: number;
    show_browser?: boolean;
  }): Promise<ToolResult<{ success: boolean; sourcesAdded?: number; message?: string }>> {
    const { query, notebook_url, session_id, timeout_ms, show_browser } = args;
    ...
    try {
      // ──────────────── HARD GATE: Notebook URL Resolution ────────────────
      // RULE: 1 VIDEO = 1 MASTER NOTEBOOK. Deep Research MUST target an existing notebook.
      // NEVER fall through to creating a new notebook silently.
      let resolvedNotebookUrl = notebook_url;
      let resolvedVia = 'explicit notebook_url parameter';

      if (!resolvedNotebookUrl) {
        const activeNotebook = this.library.getActiveNotebook();
        if (activeNotebook?.url) {
          resolvedNotebookUrl = activeNotebook.url;
          resolvedVia = `active notebook: "${activeNotebook.name}" (id: ${activeNotebook.id})`;
          log.warning(`  ⚠️ notebook_url not provided — falling back to active notebook: ${activeNotebook.name}`);
        }
      }

      if (!resolvedNotebookUrl && CONFIG.notebookUrl) {
        resolvedNotebookUrl = CONFIG.notebookUrl;
        resolvedVia = 'CONFIG.notebookUrl (env NOTEBOOK_URL)';
        log.warning(`  ⚠️ notebook_url not provided — falling back to CONFIG.notebookUrl`);
      }

      if (!resolvedNotebookUrl) {
        // HARD STOP: Do NOT proceed without a notebook URL. 
        // This prevents accidental creation of new notebooks.
        return {
          success: false,
          error: `⛔ HARD GATE: deep_research REQUIRES a target notebook.\n` +
            `DO NOT call deep_research without specifying which notebook to use.\n` +
            `Workflow:\n` +
            `1) Check episodes/[slug]/.notebook_url\n` +
            `2) If missing, ASK THE USER for the URL. DO NOT use create_notebook unless explicitly asked by the user.\n` +
            `3) Pass the URL to deep_research(query, notebook_url)\n`,
        };
      }

      log.info(`  ✅ Resolved notebook URL via: ${resolvedVia}`);
      log.info(`  📓 Target: ${resolvedNotebookUrl}`);
      ...
      // Import and instantiate DeepResearcher
      const { DeepResearcher } = await import('../content/deep-researcher.js');
      ...
      if (result.success) {
        log.success(`✅ [TOOL] deep_research completed: ${result.message}`);
      } else {
        log.error(`❌ [TOOL] deep_research failed: ${result.error}`);
      }

      // Save DOM dump to file for debugging
      if (result.domDump) {
        try {
          const fs = await import('fs');
          fs.writeFileSync('/tmp/notebook_dom_dump.txt', result.domDump, 'utf-8');
          log.info(`  📄 DOM dump saved to /tmp/notebook_dom_dump.txt`);
        } catch (e) {
          log.warning(`  ⚠️ Could not save DOM dump: ${e}`);
        }
      }
      ...
      return {
        ...
        data: {
          ...
          message: result.domDump
            ? `${result.message || ''}\n\nDOM dump saved to /tmp/notebook_dom_dump.txt`
            : result.message,
        },
        error: result.error,
      };
