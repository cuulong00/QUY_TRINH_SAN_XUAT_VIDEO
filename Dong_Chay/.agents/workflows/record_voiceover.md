---
description: BƯỚC BẮT BUỘC để thu âm TTS voiceover cho tập phim Dòng Chảy (từ chapter_XX.md) bằng hệ thống OmniVoice RunPod GPU chuẩn hóa.
---
...
> - Tại dự án `Dong_Chay`, thu âm được kích hoạt nhanh chóng thông qua script cầu nối:
>   ```bash
...
>   `--voice voice_mau_dong_chay_dai --speed 0.95 --temperature 0.3 --num-step 64 --no-cache`
...
1. Xác định `slug` của tập phim (ví dụ: `tap-doan-dai-dung` hoặc `episodes/tap-doan-dai-dung`).
...
- **Thay đổi tốc độ hoặc giọng đọc:**
  ```bash
  python3 scripts/record_voiceover.py [slug] --voice mc_nam_refined_short --speed 0.9
  ```
