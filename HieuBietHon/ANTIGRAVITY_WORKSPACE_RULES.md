# Antigravity Workspace Rules — Hiểu Biết Hơn
...
## Video Scene Splitting Quality Constraints (Veo 3.1 8s)
...
- **Scientific Word Count & WPS Calculation:** Thời lượng của các phân cảnh phải được ước lượng chính xác dựa trên số từ và độ dài audio thực tế của chương:
  - `WPS (Words Per Second) = Tổng số từ / Thời lượng audio thực tế` (nếu không có audio, dùng WPS mặc định = 3.58 từ/giây).
  - `Thời lượng câu = Số từ của câu / WPS`.
- **Gom và Tách Phân Cảnh:**
  - *Chương 1 (Hook):* Không được gộp câu thoại. Mỗi câu thoại là 1 phân cảnh. Nếu câu thoại `> 7.0` giây, bắt buộc phải tách đôi câu đó thành 2 sub-scenes con (`SCXXXa`, `SCXXXb`), mỗi cảnh con có prompt riêng.
  - *Chương 2 đến Chương 7:* Gom các câu thoại liên tiếp sao cho tổng thời lượng thoại `<= 7.0` giây và tối đa là 3 câu/scene. Nếu câu tiếp theo làm tổng thời lượng vượt quá 7.0 giây, hoặc một câu đơn lẻ `> 7.0` giây ➡️ Tách cảnh và tạo sub-scenes con.
- **Tag & Prompt Format:** Nhãn tag phân cảnh bắt buộc dùng tiếng Việt không dấu (ví dụ `Chuong X - Phan canh Y`). Nội dung prompt trong `video_prompts.txt` 100% không chứa ký tự tiếng Việt có dấu.