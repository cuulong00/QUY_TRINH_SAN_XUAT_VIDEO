...
def analyze_prompts(file_path):
    ...
    for line in lines:
        line_str = line.strip()
        # Skip empty lines or markdown/shell comments
        if not line_str or line_str.startswith("#"):
            continue

        # Match modern flow_batch_studio format: CHXX_SCYYY [IMAGE|VIDEO]: [@ref ->] prompt
        # OR legacy format: [CHXX_SCYYY]: [@ref ->] prompt or CHXX_SCYYY: [@ref ->] prompt
        m = re.match(
            r'^(?:\[?([A-Za-z0-9_-]+)\]?:?)\s*(?:\[(IMAGE|VIDEO)\]\s*:\s*)?(?:@([^\s\->]+)\s*->\s*)?(.*)$',
            line_str, re.IGNORECASE
        )
        if not m:
            id_rendering_errors.append(("UNKNOWN", f"Line does not follow standard ID prefix format: '{line_str}'"))
            continue

        current_id = m.group(1).strip()
        prompt_type = m.group(2).upper() if m.group(2) else None
        img_filename = m.group(3).strip() if m.group(3) else None
        desc_clean = m.group(4).strip()
        ids.append(current_id)

        # Check I2V mapping:
        # For [VIDEO] (or untyped video prompts), @ref should match current_id (e.g. @CH01_SC002.png -> CH01_SC002)
        # For [IMAGE], @ref is an external character reference asset (e.g. @lanhdao_tolam.jpg), which is NOT required to match current_id.
        if img_filename:
            img_id = os.path.splitext(img_filename)[0]
            if prompt_type == 'VIDEO' or prompt_type is None:
                if img_id.lower() != current_id.lower():
                    mapping_errors.append((current_id, f"Video source image ID '{img_id}' does not match line ID '{current_id}'!"))

        # Clean trailing generation flags like --ar 16:9 --dur 8s
        desc_clean = re.sub(r'--ar\s+[0-9:]+', '', desc_clean, flags=re.IGNORECASE)
        desc_clean = re.sub(r'--dur\s+[0-9]+s?', '', desc_clean, flags=re.IGNORECASE).strip()
        ...
        # Exclude current_id and image filenames
