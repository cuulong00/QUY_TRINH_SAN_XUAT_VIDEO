...
export async function humanType(
  ...
  if (!CONFIG.stealthEnabled || !CONFIG.stealthHumanTyping || text.length > 50) {
    // Fast typing without stealth (force to bypass any overlays) for long texts to prevent browser hangs and API timeouts
    await page.fill(selector, text, { force: true });
    // Trigger input and change events to ensure Angular/React components register the value change
    await page.evaluate((sel) => {
      const doc = (globalThis as any).document;
      const el = doc ? doc.querySelector(sel) : null;
      if (el) {
        const evClass = (globalThis as any).Event;
        el.dispatchEvent(new evClass('input', { bubbles: true }));
        el.dispatchEvent(new evClass('change', { bubbles: true }));
      }
    }, selector);
    return;
  }
  ...
  // Clear existing text first (force to bypass citation tooltip overlays)
  await page.fill(selector, '', { force: true });
  await randomDelay(30, 80);

  // Click to focus — use force:true to bypass any overlays
  // (citation-tooltip-footer from previous responses can block normal clicks)
  ...
  while (i < text.length) {
    ...
    if (withTypos && Math.random() < 0.003 && i > 0) {
      ...
      await page.fill(selector, currentText, { force: true });
