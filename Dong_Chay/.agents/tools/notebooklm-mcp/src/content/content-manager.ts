...
export class ContentManager {
  ...
  private async clickAddSource(): Promise<void> {
    // Check if the add source popup or list of choices is already visible
    // (e.g. on empty notebooks or when ?addSource=true is active)
    const uploadDialogSelectors = [
      '[role="dialog"]',
      '.upload-dialog',
      '.modal',
      '[data-dialog="upload"]',
      ...
      'span:has-text("Tải tệp lên")',
      'span:has-text("Paste text")',
      'span:has-text("Văn bản sao chép")'
    ];
    for (const sel of uploadDialogSelectors) {
      try {
        if (await this.page.locator(sel).first().isVisible({ timeout: 500 }).catch(() => false)) {
          log.info(`  ℹ️ Upload dialog or choices already visible (${sel}), skipping clickAddSource`);
          return;
        }
      } catch { /* ignore */ }
    }
