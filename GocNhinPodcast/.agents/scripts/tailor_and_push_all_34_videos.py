...
# 100% TAILOR-MADE INDIVIDUAL CITATIONS MAP FOR ALL 34 VIDEOS
TAILORED_DB = {
    "gUQAk_R3wgs": [
...
def format_tailored_block_6(citations):
    disclaimer = (
        "--------------------------------------------------\n"
...
def main():
    ...
    print(f"🚀 Pushing 100% TAILOR-MATCHED citations to ALL 34 videos via YouTube API...\n")
    ...
    for vid, cits in TAILORED_DB.items():
        try:
            ...
            block_6 = format_tailored_block_6(cits)
            ...
            print(f"  ✅ SUCCESS: Tailored & updated Video ID {vid} [{title[:35]}...]")
            updated += 1
            
        except Exception as e:
            print(f"  ❌ Error updating {vid}: {e}")
            
    print(f"\n🎉 COMPLETED: Successfully tailored & updated {updated}/34 videos on YouTube!")
