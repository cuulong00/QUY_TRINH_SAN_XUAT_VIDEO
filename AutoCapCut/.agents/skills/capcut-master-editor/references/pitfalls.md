...
## Never rely on `volume: 0.0` alone to mute or separate audio from video clips

Setting `volume: 0.0` on a video segment merely lowers the software volume slider in CapCut. If the source MP4 file contains an audio stream:
- CapCut Desktop will still show the clip with an active audio stream/waveform icon.
- `materials.videos` retains `has_audio: true` and `has_sound_separated: false`.
- Any external player, preview engine, or relink action can still process or leak the ambient audio.

**Mandatory Fix:** Always physically strip audio streams using `ffmpeg -v error -y -i <video> -an -c:v copy <stripped>` and explicitly set `has_audio: false` and `has_sound_separated: true` in `materials.videos`.
