  ...
  if (opts.checkLocalPaths) {
    ...
    for (const kind of ["videos", "audios"] as const) {
      for (const mat of draft.materials[kind] ?? []) {
        ...
        if (opts.draftDir && resolvedPath.includes("##_draftpath_placeholder_")) {
          resolvedPath = resolvedPath.replace(/##_draftpath_placeholder_[^#]+_##/g, opts.draftDir);
        }
        ...
        const probe = probeMedia(resolvedPath, probeCmd);
        if (probe === null) {
          issues.push({
            severity: "info",
