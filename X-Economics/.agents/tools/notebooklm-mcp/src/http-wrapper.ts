...
// Deep research
app.post('/deep-research', async (req: Request, res: Response) => {
  const reqId = req.requestId.substring(0, 8);
  try {
    const { query, notebook_url, session_id, timeout_ms, show_browser } = req.body;

    if (!query) {
      log.warning(`[${reqId}] /deep-research - Missing query`);
      return res.status(400).json({
        success: false,
    ...
    const result = await toolHandlers.handleDeepResearch({
      query,
      notebook_url,
      session_id,
      timeout_ms,
    ...
    log.success(`[${reqId}] /deep-research - Completed`);
    res.json(result);
  } catch (error) {
    log.error(`[${reqId}] /deep-research - Error: ${error instanceof Error ? error.message : String(error)}`);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : String(error),
    });
...
async function startServer(port: number, host: string): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Set server timeouts to 30 minutes to allow long-running Deep Research
    server.timeout = 1800000;
    server.keepAliveTimeout = 1800000;
    server.headersTimeout = 1800000;
    if ('requestTimeout' in server) {
      (server as any).requestTimeout = 1800000;
    }
