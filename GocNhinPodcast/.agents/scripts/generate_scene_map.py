...
def split_sentence_by_duration(s, wps):
    ...
    # Try splitting at comma, colon, semicolon, or dash that has at least 6 words on each side
    mid = len(s) / 2
    valid_commas = [m.start() for m in re.finditer(r'[,:;\-–—]', s) 
                    if len(s[:m.start()].split()) >= 6 and len(s[m.start():].split()) >= 6]
    if valid_commas:
...
def process_episode(episode_dir):
    ...
    for i in range(1, 21):
        ...
        if is_visual:
            ...
            for line in lines:
                # Support single-line table row: | **CH01_SC001** | Thoại | Bối cảnh | Text overlay |
                m_table_row = re.match(r'^\|\s*\*\*([A-Za-z0-9_]+)\*\*\s*\|\s*(.*?)\s*\|\s*(.*?)\s*\|\s*(.*?)\s*\|?$', line)
                if m_table_row:
                    row_id = m_table_row.group(1)
                    row_spoken = m_table_row.group(2).strip()
                    row_summary = m_table_row.group(3).strip()
                    row_overlay = m_table_row.group(4).strip().replace('`', '')
