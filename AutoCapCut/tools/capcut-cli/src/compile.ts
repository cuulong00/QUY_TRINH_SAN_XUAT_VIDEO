...
export function compileDraft(spec: CompileSpec, opts: CompileOptions): CompileResult {
  ...
  for (const track of spec.tracks) {
    for (const item of track.items) {
      ...
      const effectiveDuration = item.speed !== undefined ? Math.round(duration * item.speed) : duration;
      if (
        item.duration !== undefined &&
        media?.durationUs &&
        item.type !== "photo" &&
        effectiveDuration > media.durationUs + 10_000
      ) {
        throw new Error(
          `compile: duration for ${item.path} exceeds source duration (${effectiveDuration} > ${media.durationUs}us)`,
        );
      }
      if (track.type === "video") {
