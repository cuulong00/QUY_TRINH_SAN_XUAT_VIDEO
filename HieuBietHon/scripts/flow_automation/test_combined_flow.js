...
async function setModel(page, modelName) {
  ...
  // Find model menu dropdown button
...
async function clearReferenceImages(page) {
  console.log(`[REF_IMAGE] Checking if reference images need to be cleared...`);
  const hasRef = await page.evaluate(() => {
    const textbox = document.querySelector('[role="textbox"]');
    if (!textbox) return false;
    const container = textbox.closest('div.sc-26b30722-0, div.sc-c9e4708a-0');
    if (!container) return false;
    // Look for image attachment tag
    return !!container.querySelector('img, [aria-label="Remove content"], [aria-label="Xoá nội dung"]');
  });
  
  if (hasRef) {
    console.log(`[REF_IMAGE] Found attached reference image. Clearing...`);
    const removeBtnHandle = await page.evaluateHandle(() => {
      const textbox = document.querySelector('[role="textbox"]');
      if (!textbox) return null;
      const container = textbox.closest('div.sc-26b30722-0, div.sc-c9e4708a-0');
      if (!container) return null;
      return container.querySelector('button[aria-label="Remove content"], button[aria-label="Xoá nội dung"]');
    });
    const removeBtn = removeBtnHandle ? removeBtnHandle.asElement() : null;
    if (removeBtn) {
      await clickNative(page, removeBtn);
      await delay(1000);
    }
  } else {
    console.log(`[REF_IMAGE] No reference image currently attached.`);
  }
}
...
async function main() {
    let browser;
    try {
        console.log('Connecting to browser debug port 9222...');
        ...
        const imagePrompt = 'A massive rough diamond sparkling on a dark charcoal velvet cloth, cinematic editorial illustration style, minimalist graphic novel aesthetic.';
        await insertPromptViaSlate(page, imagePrompt);
        await page.screenshot({ path: `${scratchDir}/test_combined_1_image_prompt.png` });
        console.log('Screenshot saved: test_combined_1_image_prompt.png');

        console.log('3. Clicking submit for image...');
        const oldTilesImage = await getExistingTileIds(page);
        await clickSubmitButton(page);
        const newImageTileId = await getNewTileId(page, oldTilesImage);
        console.log(`Image submitted! Tile ID: ${newImageTileId}`);
        await page.screenshot({ path: `${scratchDir}/test_combined_2_image_submitted.png` });
        console.log('Screenshot saved: test_combined_2_image_submitted.png');
        
        // Wait 15 seconds to simulate cooldown and check progress
        console.log('Waiting 15 seconds cooldown...');
        await delay(15000);
        
        console.log('=== PART 2: TESTING VIDEO CREATION ===');
        console.log('4. Setting model to Veo 3.1...');
        await setModel(page, 'Veo 3.1 - Lite [Lower Priority]');
        await setBatchSizeToOne(page);
        
        console.log('5. Adding reference image (reusing successful CH01_SC001 image URL)...');
        ...
        await page.screenshot({ path: `${scratchDir}/test_combined_3_video_ref_added.png` });
        console.log('Screenshot saved: test_combined_3_video_ref_added.png');

        console.log('6. Typing video prompt...');
        ...
        await page.screenshot({ path: `${scratchDir}/test_combined_4_video_prompt.png` });
        console.log('Screenshot saved: test_combined_4_video_prompt.png');

        console.log('7. Clicking submit for video...');
        const oldTilesVideo = await getExistingTileIds(page);
        await clickSubmitButton(page);
        const newVideoTileId = await getNewTileId(page, oldTilesVideo);
        console.log(`Video submitted! Tile ID: ${newVideoTileId}`);
        await page.screenshot({ path: `${scratchDir}/test_combined_5_video_submitted.png` });
        console.log('Screenshot saved: test_combined_5_video_submitted.png');

        console.log('8. Waiting 10 seconds to see state...');
        await delay(10000);
        await page.screenshot({ path: `${scratchDir}/test_combined_6_final_state.png` });
        console.log('Screenshot saved: test_combined_6_final_state.png');

        console.log('SUCCESS! Combined test flow completed.');

    } catch (error) {
        console.error('Error during combined test flow:', error);
    } finally {
        if (browser) {
            browser.disconnect();
        }
