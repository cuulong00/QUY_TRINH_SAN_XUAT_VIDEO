...
> `[Absolute Path: /Users/pro16/Documents/VideoProject/KenhTamLy/.agents/personas/the_scene_architect.md]`
>
...
1. **Giao thức Đồng bộ Toán học (Bắt buộc):** Để tương thích với clip Veo 3.1 dài cố định 8.0s, thời lượng thoại thực tế tối đa cho mỗi phân cảnh/sub-scene con bắt buộc phải **<= 7.0 giây** (chừa lại 1.0 giây hình dự phòng). Với tốc độ đọc của narrator kênh KenhTamLy trung bình là **3.81 từ/giây**, điều này tương đương với **giới hạn cứng tối đa 26 từ thoại tiếng Việt cho mỗi phân cảnh hoặc phân cảnh phụ**.
...
2. **BƯỚC BẮT BUỘC 1 (HARD GATE):** Bằng mọi giá, bạn PHẢI dùng tool `run_command` để chạy đoạn mã Python CỐ ĐỊNH của hệ thống như sau (thay đường dẫn `[slug]` tương ứng):
   ```bash
   python3 /Users/pro16/Documents/VideoProject/KenhTamLy/.agents/scripts/generate_scene_map.py /Users/pro16/Documents/VideoProject/KenhTamLy/episodes/[slug]
   ```
