...
export function addTransition(
  ...
  transitions.push({
    category_id: (meta as any).category_id || "2037709433",
    category_name: (meta as any).category_name || "Classic",
    duration: dur,
    effect_id: meta.resource_id || meta.effect_id,
    id,
    is_overlap: meta.is_overlap ?? true,
    name: meta.name,
    platform: "all",
    resource_id: meta.resource_id,
    source_platform: 1,
    third_resource_id: meta.resource_id,
    type: "transition",
  });
...
export function requireTextMaterial(
  ...
  const seg = found.segment;
  let text = findMaterial(draft.materials.texts as MaterialText[], seg.material_id);
  if (!text && (draft.materials as unknown as { text_templates?: Array<{ id: string; text_info_resources?: Array<{ text_material_id?: string }> }> }).text_templates) {
    const tmpls = (draft.materials as unknown as { text_templates: Array<{ id: string; text_info_resources?: Array<{ text_material_id?: string }> }> }).text_templates;
    const tmpl = findMaterial(tmpls as unknown as MaterialText[], seg.material_id);
    const textMatId = tmpl ? (tmpl as unknown as { text_info_resources?: Array<{ text_material_id?: string }> }).text_info_resources?.[0]?.text_material_id : undefined;
    if (textMatId) {
      text = findMaterial(draft.materials.texts as MaterialText[], textMatId);
    }
  }
