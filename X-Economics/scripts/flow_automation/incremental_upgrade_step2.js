...
async function run() {
    ...
    try {
        ...
        // Chờ ô chat Tool Builder xuất hiện
        ...
        if (!chatCoords) {
            console.error('Không tìm thấy ô chat Tool Builder bên phải.');
            return;
        }
        ...
        // Lệnh 2 viết lại khéo léo không dùng từ khóa nhạy cảm
        const command2 = "Please update Sidebar.tsx and App.tsx: Add a button labeled 'Select Local Folder' below the model dropdowns. When clicked, let the user select a local directory. Write code so that whenever a track asset finishes generation, the tool automatically saves the resulting asset file into this selected directory, using the scene ID as the filename (e.g., 'c1-1.mp4' or 'c1-2.png').";
        
        console.log(`Gửi câu lệnh nâng cấp bước 2 (đã sửa): "${command2}"`);
        await flowPage.keyboard.type(command2, { delay: 0 });
        await delay(2000);
        ...
        console.log('Đang chờ AI bắt đầu cập nhật code bước 2...');
        await delay(8000);
        ...
        while (isWorking && checkCount < maxChecks) {
            ...
            console.log(`AI đang cập nhật code bước 2... (Lần kiểm tra ${checkCount}) - Trạng thái: ${isWorking ? 'Đang chạy' : 'Đã hoàn tất'}`);
            await delay(5000);
        }

        console.log('AI đã cập nhật xong bước 2!');
        await delay(3000);

        // Chụp ảnh màn hình kết quả bước 2
        const screenshotPath = path.resolve('/Users/pro16/.gemini/antigravity/brain/98676974-b36c-4473-904a-ca038cc18ff4/upgrade_step2_success.png');
        await flowPage.screenshot({ path: screenshotPath });
        console.log(`Đã lưu ảnh chụp màn hình kết quả bước 2 tại: ${screenshotPath}`);
        ...
        if (doneBtnCoords) {
            ...
            await delay(4000);
            console.log('Đã lưu thành công bước 2!');
        }

    } catch (error) {
        console.error('Lỗi khi nâng cấp bước 2:', error);
    } finally {
        if (browser) {
            browser.disconnect();
        }
