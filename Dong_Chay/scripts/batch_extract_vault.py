...
def run_query(idx, item):
    fname = item["filename"]
    ftitle = item["title"]
    fprompt = item["prompt"]
    outpath_md = os.path.join(OUTPUT_DIR, fname)
    outpath_json = os.path.join(OUTPUT_DIR, fname.replace(".md", ".json"))

    if os.path.exists(outpath_md) and os.path.getsize(outpath_md) > 500:
        print(f"[{idx}/{len(QUERIES)}] ⏭️ Đã tồn tại: {fname}, bỏ qua.")
        return

    print(f"\n[{idx}/{len(QUERIES)}] 📝 Đang truy vấn NotebookLM: {ftitle}...")
    cmd = [
        NOTEBOOKLM_BIN,
        "ask",
        fprompt,
    ...
    try:
        ...
        if res.returncode != 0:
            print(f"    ❌ Lỗi CLI (code {res.returncode}): {res.stderr}")
            return
        ...
        # Ghi JSON sidecar
        with open(outpath_json, "w", encoding="utf-8") as fj:
            fj.write(res.stdout)

        # Ghi Markdown với trích dẫn chi tiết
        with open(outpath_md, "w", encoding="utf-8") as fm:
            fm.write(f"# {ftitle}\n\n")
            fm.write(f"<!--\nTRÍCH XUẤT NGUỒN GỐC RAG NOTEBOOKLM:\n- Master Notebook ID: {NOTEBOOK_ID}\n- File Đích: {fname}\n- Số trích dẫn xác minh: {len(references)} citations\n-->\n\n")
            fm.write(f"## 1. NỘI DUNG TỔNG HỢP TỪ CƠ SỞ DỮ LIỆU ĐỐI SOÁT\n\n")
            fm.write(answer)
            fm.write(f"\n\n## 2. BẢNG TRÍCH DẪN NGUỒN THỰC CHỨNG (GROUND-TRUTH CITATIONS)\n\n")
            if references:
                for ref in references:
                    c_num = ref.get("citation_number", "")
                    c_text = ref.get("cited_text", "").strip().replace("\n", " ")
                    s_id = ref.get("source_id", "")
                    fm.write(f"* **[{c_num}]** `{s_id}`: *\"{c_text[:300]}...\"*\n")
            else:
                fm.write("*Không có trích dẫn trực tiếp.*\n")
            fm.write(f"\n---\n*Trích xuất tự động qua Google NotebookLM Direct RPC API từ Master Notebook {NOTEBOOK_ID}*\n")

        print(f"    ✅ Hoàn tất trích xuất {fname} ({len(answer)} ký tự, {len(references)} citations)!")

    except Exception as e:
        print(f"    ❌ Lỗi ngoại lệ {fname}: {e}")

def main():
    print(f"=== BẮT ĐẦU BATCH EXTRACTION {len(QUERIES)} CHUYÊN ĐỀ VÀO RESEARCH_VAULT ===")
    for idx, item in enumerate(QUERIES, 1):
        run_query(idx, item)
        time.sleep(2)
    print("\n🎉 HOÀN TẤT TOÀN BỘ BATCH EXTRACTION VÀO RESEARCH_VAULT!")
