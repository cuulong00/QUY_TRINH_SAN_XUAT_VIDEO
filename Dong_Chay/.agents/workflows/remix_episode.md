---
description: Tuyến Remix — Tạo kịch bản mới từ video YouTube có sẵn, viết lại sáng tạo theo voice Dòng Chảy
---
...
3. Lập bảng phân tích theo template trong guide:
   ...
   - So sánh đối tượng video gốc vs đối tượng Dòng Chảy
...
6. Dựng `05_thesis_map.md` + `07_outline.md` + `08_chapter_briefs.md`.
...
2. **QUY TẮC ĐẶC BIỆT CHO REMIX:**
   ...
   - Viết lại hoàn toàn bằng voice Dòng Chảy.
   - Tất cả case study, số liệu bổ sung phải MỚI (không lấy từ transcript gốc).
   - **TRANSITION BRIDGES:** Không viết tiêu đề chương. Áp dụng Seeding & Harvesting để nối ý mượt mà.
4. Tuần tự viết `chapter_01.md` → `chapter_N.md`.
5. Cập nhật các file state sau mỗi chương: `09_narrative_state_tracker.md` (NST), `10_claim_ledger.md`, `07_golden_lines.md`.
6. Sau khi viết xong tất cả → kiểm tra tổng số từ vs Depth Score target.
*Tuyệt đối không dừng hỏi giữa chừng.*

---

### BƯỚC 6: POLISH CHƯƠNG THOẠI (KHÔNG GỘP FILE)
1. Kích hoạt kỹ năng `oral_polisher`.
2. Thực hiện tối ưu hóa nhịp điệu đọc trực tiếp trên từng file `chapter_XX.md` theo chuẩn máy đọc TTS. **Không tạo file gộp `final_voiceover.md`**.
3. Kiểm tra: đọc nối tiếp các chương để đảm bảo mạch chuyển ý và logic của transition bridge mượt mà.
...
2. **Kiểm tra trùng lặp BỔ SUNG:**
   - So sánh các tệp `chapter_XX.md` với `00_raw_transcript.txt`
   - Không có câu thoại nào giống > 7 từ liên tiếp
   - Không có case study nào trùng lặp
   - Cấu trúc luận điểm khác biệt rõ ràng so với video gốc
4. Chạy Checklist 7 điểm chống trùng lặp (trong `remix_differentiation_guide.md`).
5. Sinh báo cáo `production_notes.md`.
6. 🛑 **CHECKPOINT 4:** Gửi các file `chapter_XX.md` đã được QA và báo cáo trùng lặp cho user. Đợi lệnh "Chốt / Thu âm đi".
...
3. **Thu âm:**
   ...
   - Chạy thu âm trực tiếp từng chương: `node scripts/tts/record_episode.js [slug]`
