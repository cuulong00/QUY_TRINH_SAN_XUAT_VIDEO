...
// turbo
1. `view_file` → `/Users/pro16/Documents/VideoProject/Dòng Chảy/.agents/personas/the_macro_financial_researcher.md`
2. `view_file` → `/Users/pro16/Documents/VideoProject/Dòng Chảy/.agents/skills/deep_researcher/SKILL.md`
3. `view_file` → `/Users/pro16/Documents/VideoProject/Dòng Chảy/.agents/skills/notebooklm_librarian/SKILL.md`
...
2. **Thiết kế & Hiển thị Prompt Nạp nguồn Cấu trúc (Structured Ingestion Prompt Design):**
   - Thiết kế và hiển thị một Prompt tìm kiếm tổng hợp duy nhất bằng ngôn ngữ tự nhiên, được cấu trúc bằng các đề mục và phân dòng rõ ràng để hướng dẫn NotebookLM tự động quét web tìm tài liệu cho mọi góc tiếp cận (vụ án, cơ chế vĩ mô, đối sánh lịch sử).
3. **Thiết kế & Hiển thị Danh sách Câu hỏi Trích xuất (Extraction Queries List):**
   - Thiết kế và hiển thị danh sách 8-12 câu hỏi trích xuất cụ thể tương ứng với từng chương của Khung tuyến kịch bản, đính kèm nhãn chương (`Target Chapter`).
