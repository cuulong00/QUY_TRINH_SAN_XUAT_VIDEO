...
export class BrowserSession {
  ...
  async ask(
    ...
    const askOnce = async (): Promise<AskResult> => {
      ...
      // Dismiss citation tooltips and other overlays before typing
  ...
  private async dismissTransientUi(): Promise<void> {
    ...
    // Strategy 1: Press Escape to close any focused popups/menus
    ...
    // Strategy 2: Dismiss citation-tooltip overlays
    // After NotebookLM renders a response, citation markers create
    // tooltip overlays (div.citation-tooltip-footer) inside cdk-overlay-container.
    // These overlays intercept pointer events on the textarea below.
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const hasCitationTooltip = await this.page.evaluate(`
        (() => {
          const overlays = document.querySelectorAll('.cdk-overlay-pane');
          let removed = 0;
          for (let i = 0; i < overlays.length; i++) {
            const overlay = overlays[i];
            const hasCitation = overlay.querySelector('.citation-tooltip-footer') ||
                                overlay.querySelector('.citation-tooltip');
            if (hasCitation) {
      ...
      `) as number;
      if (hasCitationTooltip > 0) {
        log.info(`  🧹 Dismissed ${hasCitationTooltip} citation tooltip overlay(s)`);
      }
    } catch {
      // Ignore errors
    }

    // Strategy 3: Click empty area to dismiss any hover-triggered tooltips
    try {
      // Click on the main content area (away from any interactive elements)
